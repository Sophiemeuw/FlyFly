...
class PreprogrammedSteps:
    ...
    def __init__(
        ...
        for leg in self.legs:
            ...
            # Backward gait is mirrored phase
            reversed_angles = joint_angles[:, ::-1]
            self._backward_psi_funcs[leg] = CubicSpline(
                phase_grid, reversed_angles, axis=1, bc_type="periodic"
            )
    ...
    def get_joint_angles(self, leg, phase, magnitude=1, reverse=False):
        ...
        psi_func = self._backward_psi_funcs[leg] if reverse else self._psi_funcs[leg]
